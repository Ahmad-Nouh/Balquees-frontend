...
export class PaintMixesComponent implements OnInit, AfterViewInit {
  ...
  handleEdit(form: any): void {
    this.paintMixesService.updatePaintMix(this.newPaintMix._id, this.newPaintMix)
        .subscribe((result) => {
          this.resetForm(form);
          const index = this.paintMixes.findIndex((paintMix: PaintMix) => paintMix._id === this.newPaintMix._id);
          this.paintMixes.splice(index, 1, result);
          this.loadTableData(this.paintMixes);
          this.totalQuantity = 0;
          this.isFlipped = false;
        });
    ...
    this.settings = {
      ...
      columns: {
        createdAt: {
          ...
          valuePrepareFunction: (value) => {
            // const newVal = moment(value.createdAt).format('llll');
            // return newVal;
            return value;
          },
          filter: {
          ...
          filterFunction: (cell: any, search?: string) => {
            ...
            if (range1.start && range1.end) {
              ...
              const endDate = moment(range1.end);
